...
output_folder <- "/Users/jackey/Desktop/Chinese_Temp_Predictions/Results"
...
for (week in 0:12) {
  ...
  vinitial <- vgm(psill = 10, model = "Sph", range = 500000, nugget = 1)  # Range in meters
  ...
  for (i in 1:nrow(my_data_sf)) {
    ...
    v_cv <- variogram(temp_diff ~ 1, data = training_data)
    vinitial_cv <- vgm(psill = 10, model = "Sph", range = 500000, nugget = 1)
    vfit_cv <- fit.variogram(v_cv, vinitial_cv)
    
    kriging_model <- gstat(formula = temp_diff ~ 1, data = training_data, model = vfit_cv)
  ...
  # Combine CV results into one data frame
  cv_results_df <- do.call(rbind, cross_val_results)
  
  # --- (A) Optionally cap the observed/predicted for plotting consistency ---
  ...
  # Compute theoretical variogram line for plotting
  ...
  # Convert to sf and CAP at -6 to 10
  kpred_sf <- st_as_sf(kpred) %>%
    mutate(
      var1.pred = pmin(pmax(var1.pred, -6), 10)
    )
  
  # Create kriging prediction plot with smaller dots and color scale from -6 to 10
  prediction_plot <- ggplot() +
    # Add size = 0.5 (or any smaller value you prefer)
    geom_sf(data = kpred_sf, aes(color = var1.pred), size = 0.5) +
    scale_color_gradientn(
      name = "Predicted",
      colors = c("blue", "white", "red"),
      values = scales::rescale(c(-6, 0, 10)),
      limits = c(-6, 10),
      breaks = seq(-6, 10, by = 2),
      labels = scales::label_number()
    ) +
    labs(
      title = paste("Predicted Temperature Difference - Week", week)
    ) +
    theme_minimal() +
    # Optionally remove latitude/longitude labels:
    theme(
  ...
  # Actual vs. Predicted plot, with axis from -6 to 10
  ...
  # Variance map with smaller dots
  variance_plot <- ggplot() +
    # Again, set size = 0.5 to make them smaller
  ...
  ggsave(
    filename = paste0(output_folder, "Combined_Plot_Temp_Week_", week, ".png"),
    plot = combined_plot,
    width = 16, height = 12
  )
}
